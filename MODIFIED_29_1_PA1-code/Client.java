...
public class Client extends Thread { 
     ...
     */
     public void sendTransactions()
     {
         ...
         {  
            ...
            {
            	System.out.println("\n --- CLient yielding cos InBuffer full --- ");
            	Thread.yield();
     ...
     */
     public void receiveTransactions(Transactions transact)
     {
         ...
         while (i < getNumberOfTransactions())
         {     
        	 while( objNetwork.getOutBufferStatus().equals("empty") && objNetwork.getServerConnectionStatus().equals("connected"))
        	{
        		 //System.out.println("\n --- CLient yielding cos OutBuffer empty and ServerConnection is connected --- ");
        		 Thread.yield();
    ...
    {   
    	...
    	// send transact to the buffer
    	if (getClientOperation().equalsIgnoreCase("sending"))
    	{
    		// Start timer
    		sendClientStartTime = System.currentTimeMillis();
    		
        	// Write out run status
        	System.out.println("\n DEBUG : Client.run() - starting client sending thread " + objNetwork.getClientConnectionStatus());
        	
        	objNetwork.setClientSenderStatus(true);
        	
    		sendTransactions();

    		objNetwork.setClientSenderStatus(false);
